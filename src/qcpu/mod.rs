...
use crossterm::crossterm_style::{paint, Color, StyledObject};
...
impl QCPU {
    ...
    pub fn print(&mut self) {
        while self.pc < self.memory.len() {
            ...
            s.push_str(&(match arity {
                0 => format!(""),
                1 => format!("{}", addressing_modes.0.to_string()),
                2 => format!("{}{}", addressing_modes.0.to_string(),
                             addressing_modes.1.to_string()),
                3 => format!("{}{}{}", addressing_modes.0.to_string(),
                             addressing_modes.1.to_string(),
                             addressing_modes.2.to_string()),
                4 => format!("{}{}{}{}", addressing_modes.0.to_string(),
                             addressing_modes.1.to_string(),
                             addressing_modes.2.to_string(),
    ...
    pub fn run(&mut self) {
        loop {
            ...
            let zip = self.zip_args(&self.memory[self.pc..self.pc + arity as usize + 1], addressing_modes);
    ...
    pub fn write(&mut self, to: &OpArgs, val: u16) {
        match to.mode {
            ...
            AddressingMode::INDIRECT => self.memory[self.read(&OpArgs { value: to.value, mode: AddressingMode::REGISTER }) as usize] = val,
    ...
    pub fn read(&self, from: &OpArgs) -> u16 {
        match from.mode {
            ...
            AddressingMode::INDIRECT => self.memory[self.read(&OpArgs { value: from.value, mode: AddressingMode::REGISTER }) as usize],
    ...
    fn zip_args(&self, arguments: &[u16], modes: (AddressingMode,
                                                  AddressingMode,
                                                  AddressingMode,
                                                  AddressingMode)) -> Vec<OpArgs> {
        let mut v = Vec::<OpArgs>::new();

        for i in 0..arguments.len() {
            match i {
                0 => {}
                1 => v.push(OpArgs { value: arguments[0], mode: modes.0 }),
                2 => v.push(OpArgs { value: arguments[1], mode: modes.1 }),
                3 => v.push(OpArgs { value: arguments[2], mode: modes.2 }),
                4 => v.push(OpArgs { value: arguments[3], mode: modes.3 }),
    ...
    pub fn execute_op(&mut self, opcode: OpCode, arguments: Vec<OpArgs>) {
        ...
        match opcode {
            ...
            OpCode::MOV => self.mov(&arguments[0],
                                    &arguments[1]),
            OpCode::JMP => self.jmp(&arguments[0]),
            OpCode::JEQ => self.jeq(&arguments[0],
                                    &arguments[1],
                                    &arguments[2]),
            OpCode::JNE => self.jne(&arguments[0],
                                    &arguments[1],
                                    &arguments[2]),
            OpCode::JGT => self.jgt(&arguments[0],
                                    &arguments[1],
                                    &arguments[2]),
            OpCode::JGE => self.jge(&arguments[0],
                                    &arguments[1],
                                    &arguments[2]),
            OpCode::JLT => self.jlt(&arguments[0],
                                    &arguments[1],
                                    &arguments[2]),
            OpCode::JLE => self.jle(&arguments[0],
                                    &arguments[1],
                                    &arguments[2]),
            OpCode::JSR => self.jsr(&arguments[0]),
            OpCode::RET => self.ret(),
            OpCode::ADD => self.add(&arguments[0],
                                    &arguments[1]),
            OpCode::SUB => self.sub(&arguments[0],
                                    &arguments[1]),
            OpCode::MUL => self.mul(&arguments[0],
                                    &arguments[1]),
            OpCode::MOD => self.modu(&arguments[0],
                                     &arguments[1]),
            OpCode::AND => self.and(&arguments[0],
                                    &arguments[1]),
            OpCode::ORR => self.orr(&arguments[0],
                                    &arguments[1]),
            OpCode::NOT => self.not(&arguments[0]),
            OpCode::XOR => self.xor(&arguments[0],
                                    &arguments[1]),
            OpCode::LSL => self.lsl(&arguments[0],
                                    &arguments[1]),
            OpCode::LSR => self.lsr(&arguments[0],
                                    &arguments[1]),
            OpCode::PSH => self.psh(&arguments[0]),
            OpCode::POP => self.pop(&arguments[0]),

            _ => panic!("unimplemented opcode {}", opcode.to_string())
        }
    }
    ...
    pub fn ext(&mut self, code: &OpArgs) {
        std::process::exit(self.read(code) as i32);
    }

    // 2
    pub fn sys(&mut self, val: &OpArgs) {
        match self.read(val) {
            //0x6 => print!("{}", self.colorise_string(std::char::from_u32(self.registers.x as u32).unwrap().to_string())),
            0x6 => print!("{}", std::char::from_u32(self.registers.x as u32).unwrap().to_string()),
            0x7 => self.registers.x = std::io::stdin().bytes().next().unwrap().unwrap() as u16,
            0xB => self.current_fg_color = self.registers.x,
            0xC => self.current_bg_color = self.registers.x,
            _ => panic!("unknown syscode {}", self.registers.x)
        };
    }
    ...
    pub fn jeq(&mut self, addr: &OpArgs, b: &OpArgs, c: &OpArgs) {
        if self.read(b) == self.read(c) { self.jmp(addr) };
    }
    ...
    // 16
    pub fn modu(&mut self, a: &OpArgs, b: &OpArgs) {
        let read_a = self.read(a);
        let read_b = self.read(b);
